...
async function apiCall(args, options, fullResponse = false) {
    ...
    try {
        const resData = await axios_1.default(options).then(res => {
            ...
            helper_1.mydebug(args, "apiCall: response=", res.headers);
            if (fullResponse) {
                return res;
            }
            else {
...
async function finalActions2(args, data) {
    ...
    helper_1.mydebug(args, "finalActions2, return value = ", return_value);
    return return_value;
}
...
async function getRecord(args) {
    ...
    ids.forEach(async function (id) {
        ...
        helper_1.mydebug(args, "getRecord", data);
...
async function update(args) {
    ...
    if (args.title || args.authors || args.description || args.date
        ...
        || args.zotero_link) {
        const metadataNew = await helper_1.updateMetadata(args, metadata);
